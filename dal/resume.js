...
exports.GetAll = function(callback) {
    connection.query('SELECT * FROM userResume;',
        function (err, result) {
            if(err) {
                console.log(err);
                callback(true);
...
exports.GetByID = function(resume_id, callback) {
    ...
    var query = 'SELECT * FROM duuude WHERE resume_id=\'' + resume_id + "'";
    console.log(query);
...
/* NOTE: Just like functions in other languages the parameters sent to a function do not need to have the same names
 ...
 this function.
 */
exports.Insert = function(resume_info, callback) {
     ...
     */
    var dynamic_query = 'INSERT INTO userResume VALUES (' +
        '\'' + resume_info.rName + '\' ' +
        ');';



   /* var dynamic_query = 'INSERT INTO duude VALUES (' +
        '\'' + resume_info.email + '\', ' +
        '\'' + resume_info.Apassword + '\', ' +
        '\'' + resume_info.fName + '\', ' +
        '\'' + resume_info.lName + '\', ' +
        '\'' + resume_info.cName + '\', ' +
        '\'' + resume_info.streetAddress + '\', ' +
        '\'' + resume_info.city + '\', ' +
        '\'' + resume_info.state + '\', ' +
        '\'' + resume_info.zip + '\', ' +
        '\'' + resume_info.sName + '\', ' +
...
exports.Update = function(resume_info,callback) {
    var query_data = [resume_info.rName, resume_info.account_id];
    var query = 'UPDATE userResume SET rName = ? WHERE account_id = ?';
...
exports.Delete = function(resume_id, callback) {
    var query = 'DELETE FROM userResume WHERE account_id = ' + account_id;
